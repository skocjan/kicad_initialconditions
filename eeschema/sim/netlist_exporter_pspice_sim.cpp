...
wxString NETLIST_EXPORTER_PSPICE_SIM::Component2Vector( const wxString& aName, SIM_PLOT_TYPE aType,
...
SIM_PLOT_TYPE NETLIST_EXPORTER_PSPICE_SIM::Vector2Signal( const std::string& aVector, wxString& aSignal ) const
{
    using namespace std;
    ...
    for( const auto& item : GetSpiceItems() )
    {
                ...
                NETLIST_EXPORTER_PSPICE_SIM::GetCurrents( (SPICE_PRIMITIVE) item.m_primitive ) )
        {
            ...
            aFormatter->Print( 0, ".save %s\n",
                    (const char*) Component2Vector( item.m_refName, SPT_CURRENT, current ).c_str() );
        }
    }
